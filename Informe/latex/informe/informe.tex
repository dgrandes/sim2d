...
Para aprender se usarán tres esquemas independientes, la política hallada en cada uno debe ser usada para llevar a cabo el experimento final 1 y comparar cuál esquema de aprendizaje funciona mejor.
...
\item En el Experimento 1.1 la performance experimenta una mejoría notable a partir de la mitad de la simulación. Esto es causado por la finalización de la etapa de exploración, deviniendo en una menor posibilidad de movimientos aleatorios y el aprovechamiento de la estrategia aprendida (figura \ref{figExpRes11}).

\item En el Experimento 1.2 (figura \ref{figExpRes12}) se nota claramente el punto de inflexión entre exploración y explotación. La diferencia con el Experimento 1.1 es la velocidad del agente, que es el doble. Esto efectivamente duplica la dificultad del agente para en llegar con éxito a destino.

\end{itemize}

En las \textit{matrices-Q}, (cuadros \ref{qmatrix11}, \ref{qmatrix12}, \ref{qmatrix13}) se puede ver que los valores de utilidad son los esperados en casi todos los casos. Recordar
...
En el Experimento 1.3 (figura \ref{figExpRes13}) el desempeño es peor debido a que es más dificil aprender con un obstáculo móvil, que intercepta al agente en distintos lugares.
...
Este experimento consiste en un pasillo largo y angosto, con un flujo de agentes moviendose en una dirección. El pasillo tiene 3m
de ancho y 15 m de largo. Hay 30 peatones en un extremo, donde uno usa \textit{Q-Learning}, que tienen que llegar al otro lado.
Se experimentó con distintas densidades por metro cudrado de agentes. Los agentes se mueven con la velocidad de deseo $v_{i}^{0}$.
...
política de navegación. Un esquema del experimento se encuentra en la figura \ref{figExp2}.
